...
این صفحه فهرستی از پیوندهای قرمز مورد نیاز را بر اساس موضوع نمایش می دهد ([[TYPE]]).<br/>

'''BOT_USER_NAME این فهرست را در BOT_TIME_NOW بروز کرد.'''
</div>
</center>
<center>
...
!style="background-color:#808080" align="center"|میان‌ویکی‌ها
!style="background-color:#808080" align="center"|اسم مقاله
!style="background-color:#808080" align="center"|مقاله مربوطه به زبان دیگری است
TABLE_BODY
|}
...
db = Database()
db.query = """select replace(pl_title,"کاربر:WaspBot/صفحه_تمرین/","") as page_title from pagelinks 
where pl_from in (676775)
and pl_namespace in (4)
and pl_from_namespace in (4)
and pl_title not like "%لینک_قرمز%"
order by pl_title
"""
...
for type in list_of_pages:
    type = type.replace("_", " ")
    page_name = f"کاربر:WaspBot/صفحه تمرین/{type}"
    # page_name = "مستخدم:لوقا/ملعب 34"

    url = f"https://missingtopics.toolforge.org/?language=fa&project=wikipedia&article=&category={type}&depth=1&wikimode=1&nosingles=1&limitnum=1&doit=Run"
    headers = {
    ...
    if response.status_code == 200:
        try:
            ...
            page.text = str(text).replace("TABLE_BODY", table_body).replace('BOT_USER_NAME',
                                                                            f"[[کاربر:{username_bot}|{username_bot}]]").replace(
                "BOT_TIME_NOW", "{{Transl:#time:H:i، j F Y}}").replace("TYPE", type)

            page.save("تست ربات توسط toolforge")
        except:
